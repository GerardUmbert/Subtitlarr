...
def test_disclaimer_is_first_when_first_cue_starts_at_zero():
    ...
    result = srt_io.with_ai_disclaimer([real])
    composed = _compose_and_parse(result)
    assert composed[0].content == srt_io.DISCLAIMER_TEXT
...
def test_disclaimer_survives_with_empty_subtitle_list():
    result = srt_io.with_ai_disclaimer([])
    assert len(result) == 1
    assert result[0].content == srt_io.DISCLAIMER_TEXT
...
def test_all_original_cues_preserved_after_disclaimer_insertion():
    ...
    result = srt_io.with_ai_disclaimer(subs)
    composed = _compose_and_parse(result)
